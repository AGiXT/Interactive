...
export default function MarkdownBlock({ content, chatItem, setLoading }: MarkdownBlockProps): ReactNode {
  ...
  const fileName = useMemo(() => {
    return chatItem ? `${chatItem.role}-${timestamp.split('.')[0]}` : `${timestamp.split('.')[0]}`;
  }, [chatItem, timestamp]);

  const parseMarkdownTable = useMemo(() => {
  ...
  // Memoize the processed segments to prevent re-processing during streaming
  const processedSegments = useMemo(() => {
    return textToMarkdown(content.toString());
  }, [content]);
